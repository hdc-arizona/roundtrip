...
class Interface(Magics):
    ...
    @line_magic
    def beginTest(self, line):
        self.start = time.time()
        # run load vis
        self.loadVisualization("myVis1 eval.js data.js")

    @line_magic
    def endTest(self, line):
        # run fetch data to grab the highlighted points
        resultsInPython = ""
        self.fetchData(" (myVis1, resultsInPython, getResults())")
        self.end = time.time()

    @line_magic
    def Results(self, line):
        print(self.shell.user_ns["resultsInPython"])
