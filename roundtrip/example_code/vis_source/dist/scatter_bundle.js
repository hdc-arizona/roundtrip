...
// Module
___CSS_LOADER_EXPORT___.push([module.id, ".exclude{\n    stroke: black;\n    stroke-width: 3px;\n}\n\n.selected-pt{\n    stroke:black;\n    stroke-width: 3px;\n}\n", ""]);
// Exports
...
// EXTERNAL MODULE: ./node_modules/css-loader/dist/cjs.js!./roundtrip/example_code/vis_source/Scatter/scatter.css
var scatter = __webpack_require__(483);
;// CONCATENATED MODULE: ./roundtrip/example_code/vis_source/Scatter/scatter.css
...
var update = injectStylesIntoStyleTag_default()(scatter/* default */.Z, options);




       /* harmony default export */ const Scatter_scatter = (scatter/* default */.Z && scatter/* default.locals */.Z.locals ? scatter/* default.locals */.Z.locals : undefined);

;// CONCATENATED MODULE: ./roundtrip/example_code/vis_source/Scatter/scatter.js
...
function getMinMax(vals_obj) {
  ...
  for (var _i = 0, _Object$keys = Object.keys(vals_obj); _i < _Object$keys.length; _i++) {
...
function update_subselection(selected) {
  ...
  for (var _i2 = 0, _Object$keys2 = Object.keys(scatter_data); _i2 < _Object$keys2.length; _i2++) {
    ...
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        var sel = _step.value;
        subselect[key][sel] = scatter_data[key][sel];
      }
    } catch (err) {
      _iterator.e(err);
    } finally {
...
var json = RT['scatter_src'];
var scatter_data = JSON.parse(json);

var present_continents = _toConsumableArray(new Set(Object.values(scatter_data['Continent'])));
...
for (var _i3 = 0, _Object$keys3 = Object.keys(scatter_data); _i3 < _Object$keys3.length; _i3++) {
  var key = _Object$keys3[_i3];
  var first = Object.keys(scatter_data[key])[0];

  if (!isNaN(parseFloat(scatter_data[key][first]))) {
    scaleDomain[key] = getMinMax(scatter_data[key]);
  }
}

var pivot = [];

for (var _i4 = 0, _Object$keys4 = Object.keys(scatter_data['Code']); _i4 < _Object$keys4.length; _i4++) {
  var i = _Object$keys4[_i4];
  var row = {
    id: Number(i)
  };

  for (var _i5 = 0, _Object$keys5 = Object.keys(scatter_data); _i5 < _Object$keys5.length; _i5++) {
    var _key = _Object$keys5[_i5];
    row[_key] = scatter_data[_key][i];
  }
...
var scatter_options = Object.keys(scaleDomain);
...
var brushed = [];
var valid_data = [];
var margins = chartScale.margins;
var xscale = linear_linear().range([margins.left, chartScale.width - margins.right]);
var yscale = linear_linear().range([margins.top, chartScale.height - margins.bottom]);
var colorMap = ordinal().domain(Object.keys(continentMap)).range(Tableau10);
var svg = src_select(element).select('#scatter-canvas').attr('height', chartScale.height).attr('width', chartScale.width);
var ydropdown = src_select(element).select('#y-select');
ydropdown.selectAll('.y-options').data(scatter_options).enter().append('option').attr('class', 'y-options').attr('value', function (d) {
  return d;
...
ydropdown.on('change', function () {
  yselect = ydropdown.property('value');

  if (brush_active) {
    svg.select('.brush-area').remove();
    svg.append('g').attr('class', 'brush-area').call(brsh);
  }

  brushed = [];
  render(svg);
});
var xdropdown = src_select(element).select('#x-select');
xdropdown.selectAll('.x-options').data(scatter_options).enter().append('option').attr('class', 'x-options').attr('value', function (d) {
  return d;
}).attr('selected', function (d) {
  if (d == xselect) {
    return 'true';
...
xdropdown.on('change', function () {
  xselect = xdropdown.property('value');
...
src_select(element).select("#brush").on('click', function () {
...
var axisBotSelec = svg.append("g").attr("transform", "translate(0,".concat(chartScale.height - 25, ")"));
var axisLeftSelec = svg.append("g").attr("transform", "translate(50,0)");
var brsh = brush().extent([[0, 0], [chartScale.width, chartScale.height]]).on('start', function () {
  brushed = [];
  render(svg);
  update_subselection(brushed);
}).on('brush', function (evt) {}).on('end', function (evt) {
  var area = evt.selection;
  valid_data.forEach(function (d) {
    var x = xscale(d[xselect]);
    var y = yscale(d[yselect]);
...
var legend = svg.append('g').attr('class', 'legend-grp');
legend.append('rect').attr('width', 140).attr('height', 45 + 20 * present_continents.length).attr('x', chartScale.width - chartScale.margins.right + 20).attr('fill', 'rgba(0,0,0,.1)');
legend.append('text').text('Continent').attr('font-size', 18).attr('x', chartScale.width - chartScale.margins.right + 50).attr('y', 25);

function render(svg) {
  var xdomain = [scaleDomain[xselect].min, scaleDomain[xselect].max];
  var ydomain = [scaleDomain[yselect].max, scaleDomain[yselect].min];
  ...
  var dots = svg.selectAll('.dots').data(valid_data, function (d) {
    return d.id;
  });
  dots.enter().append('circle').attr('class', 'dots').attr('r', 6).attr('cx', function (d) {
    ...
    var exjson = JSON.stringify(exclude);
  ...
  var lglines = legend.selectAll('.lline').data(present_continents);
  var line = lglines.enter().append('g').attr('transform', function (d, i) {
    return "translate(".concat(chartScale.width - chartScale.margins.right + 45, ", ").concat(20 * i + 45, ")");
  });
  line.append('circle').attr('class', 'lline ref').attr('r', 6).attr('fill', function (d) {
    return colorMap(d);
  });
  line.append('text').attr('x', 10).attr('y', 5).text(function (d) {
