...
function bin() {
  ...
  histogram.value = function(_) {
    return arguments.length ? (value = typeof _ === "function" ? _ : constant_constant(_), histogram) : value;
  };

  histogram.domain = function(_) {
    return arguments.length ? (domain = typeof _ === "function" ? _ : constant_constant([_[0], _[1]]), histogram) : domain;
  };

  histogram.thresholds = function(_) {
    return arguments.length ? (threshold = typeof _ === "function" ? _ : Array.isArray(_) ? constant_constant(slice.call(_)) : constant_constant(_), histogram) : threshold;
  };
...
var Histogram = /*#__PURE__*/function (_React$Component) {
  ...
  function Histogram(props) {
    ...
    _this.currentVar = 'Electricity from oil (TWh)';
    _this.rawdata = JSON.parse(window.Roundtrip['hist_data']);
    _this.data = _this.getDataPivot(_this.rawdata);
    var bins = bin();
    _this.data_bins = bins(Object.values(_this.rawdata[_this.currentVar]));
    _this.ydomains = _this.getScaleDomains(_this.rawdata);
    _this.bindomains = _this.getBinDomains(_this.data_bins);
    _this.agg = {};
    _this.num_bins = 10;
    _this.chartScale = {
      height: 400,
    ...
    _this.xscale = linear_linear().range([_this.margins.left, _this.chartScale.width - _this.margins.right]).domain([0, _this.num_bins]);
    _this.yscale = linear_linear().range([0, _this.chartScale.height + _this.margins.bottom]).domain([_this.bindomains.min, _this.bindomains.max]);
    return _this;
  }

  _createClass(Histogram, [{
    key: "formatAggMetrics",
    value: function formatAggMetrics(d) {
      var agg = {
        "max": {},
        "min": {},
        "mean": {}
    ...
    key: "getDataPivot",
    value: function getDataPivot(data) {
      var pivot = [];

      if (Object.keys(data).includes('Code')) {
        for (var _i = 0, _Object$keys = Object.keys(data['Code']); _i < _Object$keys.length; _i++) {
    ...
    key: "getBinDomains",
    value: function getBinDomains(bins) {
      var maxmin = {
    ...
    key: "getMinMax",
    value: function getMinMax(vals_obj) {
      var minmax = {
        "min": Number.POSITIVE_INFINITY,
        "max": Number.NEGATIVE_INFINITY
      };

      for (var _i4 = 0, _Object$keys4 = Object.keys(vals_obj); _i4 < _Object$keys4.length; _i4++) {
        var val = _Object$keys4[_i4];
    ...
    value: function draw() {
      ...
      var svg = src_select(this.refs.histCanvas).attr('height', this.chartScale.height).attr('width', this.chartScale.width);
      var bars = svg.selectAll('.bars').data(this.data_bins);
      bars.enter().append('rect').attr('class', 'bars').attr('fill', 'rgba(0,0,0,.5)').attr('width', this.chartScale.width / this.data_bins.length).attr('height', function (d) {
        return _this2.yscale(d.length);
      }).attr('x', function (d, i) {
        return _this2.chartScale.width / _this2.data_bins.length * i;
      }).attr('y', function (d) {
        return _this2.chartScale.height - _this2.yscale(d.length);
      }).on('mouseover', function (evt, d) {
        window.Roundtrip["hist_agg"] = JSON.stringify(_this2.formatAggMetrics(d));
        src_select(evt.target).attr('fill', 'rgba(20, 20, 170, .8)');
      }).on('mouseout', function (evt, d) {
        src_select(evt.target).attr('fill', 'rgba(0,0,0,.5)');
      });
    }
  }, {
...
react_dom.render( /*#__PURE__*/react.createElement(Histogram, null), document.getElementById("root"));
})();
